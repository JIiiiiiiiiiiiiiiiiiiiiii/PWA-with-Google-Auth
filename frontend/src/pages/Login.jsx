...
export default function Login() {
	...
	const submit = async (e) => {
		...
		try {
			...
			// Fetch user data after login - NO DELAYS, instant
			let user = null
			try {
				// Fetch user immediately - no delay
			...
			// Navigate immediately - user state is set synchronously
			navigate("/dashboard", { replace: true })
		} catch (err) {
			if (err.offline || !navigator.onLine) {
				setError(
					"Login requires an internet connection. Please connect to the internet and try again."
				)
			} else {
				setError(
